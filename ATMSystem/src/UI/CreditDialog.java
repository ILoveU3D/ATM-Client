...
public class CreditDialog extends JDialog{
	...
	public CreditDialog() {
		label.setFont(Constant.dialog_2);
		label2.setFont(Constant.dialog_1);
		label.setHorizontalAlignment(SwingConstants.CENTER);
		label2.setBounds(30,10, 100, 25);
		textField.setBounds(150,10 ,150, 25);
		textField.setFont(Constant.dialog_1);
		button.setFont(Constant.button_1);
		button.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				try {
					RegisterPanel.credit=Double.valueOf(textField.getText());
					dispose();
				} catch (Exception e2) {
					new Dialog("输入有误!");
				}
